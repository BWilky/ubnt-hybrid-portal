...
GATEWAY_IP="192.168.1.1"        # your UniFi gateway / main router
SECONDARY_IP=""                 # optional 2nd target (e.g. core switch);
                                # uplink counts as UP if EITHER answers

FAIL_LIMIT=5                    # consecutive failed checks (~minutes) before cutting PoE
RECOVER_OK=2                    # consecutive good checks before restoring PoE
AP_FAIL_LIMIT=3                 # consecutive failed AP pings before cycling that port
CYCLE_COOLDOWN=600              # seconds to leave a port alone after a cycle
...
EXCLUDE_PORTS=""                # space-separated, e.g. "eth4" - never touch these
