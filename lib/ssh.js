...
function connOpts(cfg, host) {
  ...
  if (cfg.ssh.privateKeyPath && fs.existsSync(cfg.ssh.privateKeyPath)) {
    o.privateKey = fs.readFileSync(cfg.ssh.privateKeyPath);
  } else if (cfg.ssh.password) {
    o.password = cfg.ssh.password;
  }
  return o;
}

function withConn(cfg, host, fn) {
...
async function checkStatus(cfg, host, renderedScript) {
  return withConn(cfg, host, async (conn) => {
...
async function getWatchdogStatus(cfg, host) {
  return withConn(cfg, host, async (conn) => {
...
async function deploy(cfg, host, renderedScript, vars) {
  return withConn(cfg, host, async (conn) => {
    const steps = [];
...
module.exports = { checkStatus, getWatchdogStatus, deploy, pooledMap, sha256, REMOTE_PATH };
