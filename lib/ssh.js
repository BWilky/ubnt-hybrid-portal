...
async function installPubkey(cfg, creds, host, pub) {
  return withConn(cfg, creds, host, async (conn) => {
    ...
    if (r.code !== 0 && !/Nothing to commit/i.test(r.stdout + r.stderr)) {
      throw new Error('key install failed: ' + (r.stderr || r.stdout).slice(0, 200));
    }
    log.info('public key installed', { host, user: creds.username });
    return true;
  });
}
...
module.exports = { haveAuth, checkStatus, getWatchdogStatus, deploy, installPubkey, ping, pooledMap, sha256, REMOTE_PATH };
