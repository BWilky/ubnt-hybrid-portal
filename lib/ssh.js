...
function parseApmap(text) {
  ...
  for (const line of String(text || '').split('\n')) {
    ...
    if (/^eth\d+$/.test(port || '') && MAC_RE.test(String(mac || '').toLowerCase())) out[mac.toLowerCase()] = port;
  }
  return out;
}
...
// Build the EdgeOS task-scheduler configuration command run over one
// `sg vyattacfg` session. Idempotent across re-deploys.
//
// Each `set`/`delete` is wrapped in `( … || true )`: the vyatta wrapper exits
// non-zero on benign conditions (a node that already exists on a re-deploy,
// warnings), and joining the statements with `&&` used to abort the whole
// chain on the first such non-zero — so on any switch already running the
// watchdog, the `delete weekly-ap-cycle` and the `commit` never ran and the
// retired cron entry lingered. Wrapping keeps the chain going to `commit`,
// while a genuine `commit` failure still stops `save`/`end` and surfaces.
function buildSchedulerCmd(vars, W) {
  const soft = (cmd) => `( ${cmd} || true )`;
  return [
    `${W} begin`,
    soft(`${W} set system task-scheduler task poe-watchdog executable path ${REMOTE_PATH}`),
    soft(`${W} set system task-scheduler task poe-watchdog interval 1m`),
    // retired: APs are now rebooted by the portal through UniFi
    soft(`${W} delete system task-scheduler task weekly-ap-cycle`),
    soft(`${W} set system task-scheduler task weekly-reboot executable path ${REMOTE_PATH}`),
    soft(`${W} set system task-scheduler task weekly-reboot executable arguments weekly-reboot`),
    soft(`${W} set system task-scheduler task weekly-reboot crontab-spec '${vars.REBOOT_CRON}'`),
    `${W} commit`,
...
async function deploy(cfg, creds, host, renderedScript, vars) {
  return withConn(cfg, creds, host, async (conn) => {
    ...
    const schedCmd = buildSchedulerCmd(vars, W);
...
async function cycleMac(cfg, creds, host, mac) {
  if (!MAC_RE.test(String(mac))) throw new Error('cycle-mac: invalid MAC ' + JSON.stringify(String(mac)).slice(0, 40));
  return withConn(cfg, creds, host, async (conn) => {
...
module.exports = { haveAuth, checkStatus, getWatchdogStatus, deploy, buildSchedulerCmd, cycleMac, installPubkey, ping, pooledMap, sha256, parseApmap, cronScheduled, REMOTE_PATH };
