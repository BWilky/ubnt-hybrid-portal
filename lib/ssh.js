...
// Ethernet port name. Guards remote shell interpolation.
const PORT_RE = /^eth\d+$/;
...
//
// The statements are separated by `;`, not `&&`. Two reasons:
//   1. The vyatta wrapper exits non-zero on benign conditions — a node that
//      already exists on a re-deploy, warnings on `end`. With `&&` the first
//      such non-zero aborted the whole chain, so on any switch already running
//      the watchdog the `delete weekly-ap-cycle` and the `commit` never ran
//      and the retired cron entry lingered ("in sync, no scheduler"). `;`
//      runs every statement regardless of the previous one's exit code.
//   2. The statements must NOT run in subshells: the wrapper keys its config
//      session to the shell, and a `( … )` subshell loses it ("calling
//      validateSetPath() without config session"). So `;` in one shell, and
//      the conditional save uses `if/then/else` rather than `( … || … )`.
function buildSchedulerCmd(vars, W) {
  return [
    ...
    `${W} delete system task-scheduler task weekly-ap-cycle`,
    ...
    `if [ ! -f /var/run/poe-watchdog/cut_ports ]; then ${W} save; fi`,
    `${W} end`,
  ].join('; ');
}
...
module.exports = {
  haveAuth, checkStatus, getWatchdogStatus, deploy, buildSchedulerCmd, cycleMac, installPubkey, ping, pooledMap,
  sha256, parseApmap, cronScheduled, REMOTE_PATH,
  PORT_RE, parsePorts, getPorts, getPortEvents, setPoe, cyclePort,
};
